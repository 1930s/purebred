...
import Storage.ParsedMail
       (parseMail, getTo, getFrom, getSubject, quotedMail)
import Types
import Error
import UI.Utils
...
replyToMail s =
  ...
  =<< case L.listSelectedElement (view (asMailIndex . miListOfMails) s) of
    Just (_, m) -> either handleErr handleMail
                   <$> (either Left (quotedMail preferredContentType) <$> runExceptT (parseMail m (view (asConfig . confNotmuch . nmDatabase) s)))
    Nothing -> pure id
  where
